...
也就是说，每个 epoch 结束后，agent 修改后的 candidate workspace 都会成为下一轮起点：

```text
H_t --agent update--> candidate H_{t+1}
checkpoint candidate as H_{t+1}
continue from H_{t+1}
```

即使本轮 `accepted=false`，下一轮也继续从这个 candidate workspace 开始。
