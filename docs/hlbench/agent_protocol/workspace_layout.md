...
- `feedback/` 是环境生成的公开 train feedback，只读；
- `tools/` 初始可以为空，agent 可以写自己的分析脚本；
- `experiments/` 保存 agent 主动运行 train rollout 产生的结果和临时分析输出。

不建议第一版放 `metadata/`。runner 状态、私有评估结果、validation/heldout seed、event trace 和 checkpoint 索引都应该在 workspace 外部，由 benchmark run directory 管理。
...
- Do not attempt to inspect validation or heldout data.
...
Train rollout:
  python -m hlbench.rollout --workspace . --split train --episodes 10 --out experiments/<name>
```
...
## `feedback/`: 环境生成的公开训练反馈

`feedback/` 由 benchmark 生成，agent 只读。

它只包含 train split 的公开反馈，不包含 validation 或 heldout 失败细节。
...
当前 epoch 开始时，benchmark 用当前 `system/` 在 train split 上运行一次 rollout，并把结果写入 `feedback/current/`。
...
历史 epoch 的公开 train feedback。它帮助 agent 比较学习过程，但仍然只包含 train 信息。
...
这样含义更清楚：这是第 3 轮外层学习开始时或结束时由 benchmark 记录的标准 train feedback。
