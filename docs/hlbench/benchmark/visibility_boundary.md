...
私有评测轨迹、隐藏答案、heldout 失败细节和 evaluator internals，不能给 agent。validation / heldout 只应持久化聚合分数。
```
...
这里的 “不能看到” 不应该实现成“先记录下来但藏起来”。第一版 HLBench 中，validation / heldout 不应生成可回放材料、逐步轨迹或失败明细。benchmark 只持久化聚合分数，用于选择、曲线和最终报告。
...
- validation / heldout 聚合分数只用于选择、报告和外部评估。
...
validation:
  score summary      report / selection only
