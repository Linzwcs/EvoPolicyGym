...
- **方法不限但 rollout 受控**：agent 提交的是 Python 代码，内部可用 PD / PPO / 搜索 / 规划 / NN 等策略计算；但所有环境交互数据必须来自 server 的 `/submit` 与 `feedback/`，不能在本地另跑环境或仿真器生成额外 rollout。
...
| 角色 | 负责 | 不负责 |
|---|---|---|
| **Agent** | 写 `system/policy.py`、决定何时 submit、analyze `feedback/`（只读） | 执行环境、不可见 held-out、不能写 `feedback/`、不能本地生成 rollout 数据、**不能触发 finalize** |
...
**唯一 rollout 通道**：环境 reset/step、reward、termination、episode length、return 等 rollout 数据只能通过 `POST /submit` 产生，并按 episode budget 扣费。Agent **MUST NOT** 在 `/submit` 之外调用本地 Gymnasium / MuJoCo / Box2D / highway 等环境、benchmark env 类、复制出的 dynamics/reward 函数或第三方 simulator 来训练、调参、打分或验证候选 policy。允许的本地行为限于语法检查、单元测试、静态分析、读取 `/task` 与已存在 `feedback/`，以及不执行环境/仿真的纯代码计算。
...
5. **禁止预算外 rollout**：agent 不能用本地环境或仿真器生成额外 episode 数据来绕过预算；所有可用于优化的真实环境交互都必须体现在 `/submit` 扣费记录与 `feedback/submit_NNN/` 中。
6. **自动 finalize + val-based 选择（finalize 时统一跑）**：`remaining_budget == 0` 时 server **MUST** 按以下顺序自动执行：
