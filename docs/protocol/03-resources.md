...
如果 host 出于工程安全需要配置 timeout，它只应限制一次 server-side rollout：训练 submit 的 execute 阶段，或隐藏 validation/final eval。它不限制 `/submit` 之前的 agent 行为，也不限制两次提交之间的间隔。无论 host 是否设置时间限制，agent 都不能在 `/submit` 之外自行运行本地环境或仿真器来生成预算外 rollout 数据。
...
`null` 表示 EvoPolicyGym 不施加该限制。生产平台可以在外层再设置 job-level timeout 和 harness 审计，但这些控制不进入协议反馈，也不应作为 agent 能力评分的一部分。
...
- Agent 自由决定何时提交、提交多少次、每次提交前如何分析代码、调用 LLM、或基于已有 `feedback/` 做纯离线计算。
- Agent **MUST NOT** 在提交前运行本地 Gymnasium/MuJoCo/Box2D/highway 环境、benchmark env 类、复制的 dynamics/reward 函数，或其它 simulator 来生成 observations、rewards、episode lengths、returns、candidate-policy scores 等 rollout 数据。
