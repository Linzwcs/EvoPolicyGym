...
| Gymnasium-Robotics | FetchReach, FetchPush, FetchSlide, FetchPickAndPlace, PointMaze, AntMaze, four Adroit Hand tasks, HandReach, Block/Egg/Pen manipulation with base, boolean-touch, and continuous-touch profiles, and FrankaKitchen (21 profiles); task-appropriate Host-only MuJoCo camera capture and one bounded replay GIF plus result manifest for every Episode | [`gymnasium_robotics/robotics/`](gymnasium_robotics/robotics/) |
| MetaWorld | All 50 MT1 `*-v3` tasks, MT10, MT50, and fixed custom MT collections; Host-only `corner2` MuJoCo camera capture and one bounded replay GIF plus result manifest for every Episode | [`metaworld/metaworld/`](metaworld/metaworld/) |
