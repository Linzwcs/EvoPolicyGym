...
_FRAME_SHAPE = (210, 160, 3)
_FRAME_BYTES = 210 * 160 * 3
_THUMBNAIL_SHAPE = (105, 80, 3)
_ACTIONS = {
    0: "noop",
    1: "fire",
    2: "right",
...
class AtariBenchmark:
    ...
    def feedback(self, episodes: Sequence[EpisodeRecord]) -> Feedback:
            ...
            frame_manifests.append(
                {
                    ...
                    "frame_artifact_sha256": hashlib.sha256(frame_artifact.content).hexdigest(),
                    "contact_sheet_artifact": (episode.contact_sheet_artifact_name),
        ...
        traced_steps = {episode.episode_index: len(episode.step_indices) for episode in traced}
        return Feedback(
            score=score,
            content={
                "summary": (f"Mean Tetris return {score:.3f} across {len(records)} Episodes."),
                "mean_return": score,
                ...
                "trace_raw_rgb_bytes_cap_total": (
                    _MAX_TRACED_EPISODES * (1 + 2 * _MAX_TRACED_STEPS_PER_EPISODE) * _FRAME_BYTES
                ),
                "trace_selection": (
...
def _episode_summary(
    ...
    *,
    ...
    return {
        ...
        "scored_return": (record.total_reward if record.policy_failure is None else failure_return),
        "steps": record.steps,
...
def _even_sample(values: Sequence[int], count: int) -> tuple[int, ...]:
    ...
    return tuple(values[index * (len(values) - 1) // (count - 1)] for index in range(count))
...
def _frame_artifact(episode: _TracedEpisode) -> Artifact:
    ...
    decision_frames = tuple(
        initial_frame
        if step_index == 0
        else _trace_frame(record.transitions[step_index - 1].step.observation)
...
def _contact_sheet_artifact(
    ...
    candidates = (
        ...
        *(
            (
                step_index,
                _trace_frame(episode.record.transitions[step_index].step.observation),
            )
            for step_index in episode.step_indices
        ),
    )
    ...
    rows = max(1, (len(selected) + _CONTACT_SHEET_COLUMNS - 1) // _CONTACT_SHEET_COLUMNS)
...
def _replay_artifact(
    ...
    candidates: tuple[
        ...
        *(
            (
                ...
                _trace_action(
                    episode.record.transitions[step_index].action
                ),
    ...
    selected_indices = tuple(sorted(required_indices))
    selected = tuple(candidates[index] for index in selected_indices)
...
def _encode_replay_gif(
    ...
    if len(content) > _MAX_REPLAY_ARTIFACT_BYTES:
        raise ValueError("Atari replay GIF exceeds its bounded artifact limit")
    return content, 1
...
def _replay_gif_bytes(
    ...
    *,
    ...
    if not frames:
        raise ValueError("Atari replay GIF requires at least one frame")
    rendered = [
...
def _trace_action(value: PolicyValue) -> int:
    if type(value) is not int or value not in _ACTIONS:
        raise ValueError("Atari trace Action is invalid")
    return value
...
def _trace_frame(value: PolicyValue) -> NDArray[numpy.uint8]:
    if (
        ...
        raise ValueError("Atari trace observation is invalid")
    return numpy.frombuffer(value.data, dtype=numpy.uint8).reshape(_FRAME_SHAPE)
...
def _png_rgb(image: NDArray[numpy.uint8]) -> bytes:
    if image.dtype != numpy.dtype("uint8") or image.ndim != 3 or image.shape[2] != 3:
        raise ValueError("Atari contact sheet image is invalid")
    height, width, _ = image.shape
    scanlines = b"".join(b"\0" + image[row].tobytes(order="C") for row in range(height))
