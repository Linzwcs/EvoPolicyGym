    ...
    *,
    ...
    for episode_index, record in enumerate(records):
        ...
        if not frames:
            manifests.append(
                {
                    "schema": "dm-control/rendered-frame-evidence/v1",
...
def _frame_evidence_artifact(
    ...
    *,
    ...
    if not frames:
        raise ValueError("dm_control frame evidence requires at least one frame")
    output = io.BytesIO()
    np.savez_compressed(
        output,
...
def _episode_frames(
    ...
    if len(frames) > VIDEO_MAX_FRAMES_PER_EPISODE:
        raise ValueError("dm_control video frame count exceeds its public bound")
...
def _frame(value: PolicyValue) -> NDArray[np.uint8]:
    if (
        ...
        raise ValueError("dm_control video metric contains an invalid RGB frame")
...
def _encode_bounded_gif(
    ...
    *,
    ...
    if not frames:
        raise ValueError("dm_control replay GIF requires at least one frame")
    ...
    while len(content) > _MAX_VIDEO_ARTIFACT_BYTES and len(selected) > 2:
        ...
        content = _gif_bytes(selected, profile=profile, scale=1)
    if len(content) > _MAX_VIDEO_ARTIFACT_BYTES:
        raise ValueError("dm_control replay GIF exceeds its public byte bound")
