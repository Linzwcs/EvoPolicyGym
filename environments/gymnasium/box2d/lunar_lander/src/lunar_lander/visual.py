"""Lossless RGB evidence and derived MP4 video for LunarLander."""
...
_SCHEMA = "gymnasium-lunar-lander/rendered-frame-evidence/v1"
...
def _episode_frames(
    ...
    if len(frames) > VISUAL_MAX_FRAMES_PER_EPISODE:
        raise ValueError("LunarLander rendered frame count exceeds its public bound")
...
def _frame(value: PolicyValue) -> NDArray[np.uint8]:
    if (
        ...
        raise ValueError("LunarLander visual metric contains an invalid RGB frame")
...
def _video_artifact(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="evopolicygym-lunar-lander-video-") as tmp:
    ...
    if len(content) > _MAX_VIDEO_ARTIFACT_BYTES:
        raise ValueError("LunarLander MP4 exceeds its public byte bound")
    if len(content) < 12 or content[4:8] != b"ftyp":
        raise ValueError("LunarLander MP4 encoder returned an invalid container")
    return Artifact(
        name=name,
