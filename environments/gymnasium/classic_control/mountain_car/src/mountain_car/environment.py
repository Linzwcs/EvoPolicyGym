...
_GOAL_POSITION = 0.5
_GOAL_VELOCITY = 0.0
_ENGINE_VELOCITY_INCREMENT = 0.001
...
class MountainCarEnvironment:
    ...
    def __init__(self, episode: EpisodeSpec) -> None:
        ...
        if episode.scenario is not None:
            raise ValueError("Mountain Car does not accept Episode scenarios")
    ...
    def reset(self) -> PolicyValue:
        ...
        if not -0.600001 <= position <= -0.399999 or velocity != 0.0:
            raise RuntimeError("Mountain Car initial-state semantics drifted")
    ...
    def step(self, action: PolicyValue) -> Step:
        ...
        if previous_observation is None:
            raise RuntimeError("Mountain Car observation is unavailable")
        ...
        if public_reward != -1.0:
            raise RuntimeError("Mountain Car reward semantics drifted")
        self._steps += 1
...
def _transition_metrics(
    ...
    *,
    ...
    engine_increment = (action - 1) * _ENGINE_VELOCITY_INCREMENT
    ...
    if not position_matches or not velocity_matches:
        raise RuntimeError("Mountain Car transition does not match public dynamics")
    goal_reached = position >= _GOAL_POSITION and velocity >= _GOAL_VELOCITY
    if terminated != goal_reached:
        raise RuntimeError("Mountain Car goal termination semantics drifted")
    if truncated != (step_count == _MAX_EPISODE_STEPS):
        raise RuntimeError("Mountain Car time-limit semantics drifted")
...
def _float_field(observation: dict[str, PolicyValue], name: str) -> float:
    ...
    if type(value) is not float:
        raise RuntimeError(f"Mountain Car returned invalid {name}")
    return value
