"""Typed, public Hopper-v5 environment configuration."""
...
class HopperConfig:
    """Parameters that define one Hopper Benchmark identity."""
    ...
    healthy_z_range: tuple[float, float | None] = (0.7, None)
    ...
    def __post_init__(self) -> None:
        ...
        _finite_range(
            self.healthy_state_range,
            name="healthy_state_range",
        )
        _upper_optional_range(
            self.healthy_z_range,
            name="healthy_z_range",
        )
        _finite_range(
            self.healthy_angle_range,
            name="healthy_angle_range",
        )
...
def _upper_optional_range(
    ...
    *,
    ...
    if type(value) is not tuple or len(value) != 2:
        raise TypeError(
            f"{name} must be an exact (float, float | None) tuple"
        )
    lower, upper = value
    if type(lower) is not float:
        raise TypeError(f"{name} lower bound must be an exact float")
    if upper is not None and type(upper) is not float:
        raise TypeError(
            f"{name} upper bound must be an exact float or None"
        )
    if not math.isfinite(lower):
        raise ValueError(f"{name} lower bound must be finite")
    if upper is not None and (
        not math.isfinite(upper) or lower >= upper
    ):
        raise ValueError(f"{name} upper bound must be finite and increasing")


__all__ = ["HopperConfig"]
