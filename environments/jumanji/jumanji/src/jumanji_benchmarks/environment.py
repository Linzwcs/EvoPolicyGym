...
class JumanjiEnvironment:
    ...
    def reset(self) -> PolicyValue:
        ...
        state, timestep = self._environment.reset(key)
    ...
    def step(self, action: PolicyValue) -> Step:
        ...
        state, timestep = self._environment.step(self._state, applied)
        observation = _timestep_observation(timestep)
        upstream_done = _timestep_last(timestep)
        self._steps += 1
        truncated = self._steps == self._config.max_episode_steps and not upstream_done
        self._state = state
        self._raw_observation = observation
        self._done = upstream_done or truncated
        return Step(
            observation=_policy_value(observation, name="observation"),
            reward=_number(getattr(timestep, "reward", _MISSING), name="reward"),
            terminated=upstream_done,
            truncated=truncated,
