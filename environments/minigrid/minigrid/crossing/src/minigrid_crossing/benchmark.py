...
def _spec(config: CrossingConfig) -> BenchmarkSpec:
    ...
    return BenchmarkSpec(
        ...
        environment_parameters={
            ...
            "natural_termination": ("enter goal cell or, for lava profiles, enter lava"),
...
def _episode_outcome(record: EpisodeRecord) -> str:
    ...
    if type(reason) is not str:
        raise ValueError("MiniGrid Crossing terminal reason is invalid")
    return reason if reason != "none" else "incomplete"


def _episode_diagnostics(record: EpisodeRecord) -> _EpisodeDiagnostics:
    if not record.transitions:
        raise ValueError("MiniGrid Crossing diagnostics require a transition")
...
def _int_metric(metrics: dict[str, PolicyValue], name: str) -> int:
    ...
    if type(value) is not int:
        raise ValueError(f"MiniGrid Crossing metric {name} is invalid")
    return value
