...
class CrossingEnvironment:
    ...
    def __init__(self, episode: EpisodeSpec, *, config: CrossingConfig) -> None:
        ...
        if episode.scenario is not None:
            raise ValueError("Crossing configuration belongs in CrossingConfig")
        self._seed = episode.environment_seed
        self._max_steps = config.max_episode_steps
        self._obstacle_type = config.obstacle_type
    ...
    def reset(self) -> PolicyValue:
        ...
        if type(horizon) is not int or horizon != self._max_steps:
            raise RuntimeError("MiniGrid Crossing returned an unexpected horizon")
    ...
    def step(self, action: PolicyValue) -> Step:
        ...
        if previous_signature is None:
            raise RuntimeError("MiniGrid Crossing observation history is unavailable")
        ...
        if hazard and self._obstacle_type != "lava":
            raise RuntimeError("MiniGrid wall Crossing terminated as a hazard")
        ...
        if not math.isclose(number, expected_reward, rel_tol=0.0, abs_tol=1e-12):
            raise RuntimeError("MiniGrid Crossing reward semantics drifted")
        if terminated != (success or hazard):
            raise RuntimeError("MiniGrid Crossing termination semantics drifted")
        if truncated != (self._steps == self._max_steps):
            raise RuntimeError("MiniGrid Crossing horizon semantics drifted")
...
def _observation(value: object) -> tuple[dict[str, PolicyValue], _Facts]:
    ...
    try:
        ...
        raise RuntimeError("MiniGrid Crossing returned invalid direction") from error
...
def _observation_signature(
    ...
    if type(image) is not TensorValue or type(direction) is not int:
        raise RuntimeError("MiniGrid Crossing public observation is invalid")
