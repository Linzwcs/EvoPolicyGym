...
class CrossingTests(unittest.TestCase):
    ...
    def test_step_feedback_exposes_safety_and_exploration(self) -> None:
        ...
        try:
            ...
            self.assertEqual(step.metrics["remaining_steps"], 323)
            ...
            self.assertIsInstance(step.metrics["hazard_visible"], bool)
    ...
    def test_feedback_privacy(self) -> None:
        ...
        trace = CrossingBenchmark().feedback((failed,)).artifacts[0].read_bytes()
    ...
    def test_baseline_solves_all_profiles_without_hazard(self) -> None:
        ...
        for profile in profiles:
            with self.subTest(profile=profile):
                benchmark = CrossingBenchmark(CrossingConfig(profile=profile))
                result = evaluate(
                    baseline_program(),
                    benchmark,
                ...
                self.assertTrue(episode_documents)
                self.assertTrue(
