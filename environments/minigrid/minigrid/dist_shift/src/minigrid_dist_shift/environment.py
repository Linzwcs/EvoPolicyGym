...
class DistShiftEnvironment:
    ...
    def __init__(self, episode: EpisodeSpec, *, config: DistShiftConfig) -> None:
        ...
        if episode.scenario is not None:
            raise ValueError("DistShift configuration belongs in DistShiftConfig")
    ...
    def reset(self) -> PolicyValue:
        ...
        if type(horizon) is not int or horizon != self._max_steps:
            raise RuntimeError("MiniGrid DistShift returned an unexpected horizon")
    ...
    def step(self, action: PolicyValue) -> Step:
        ...
        if previous_signature is None:
            raise RuntimeError("MiniGrid DistShift observation history is unavailable")
        ...
        if not math.isclose(number, expected_reward, rel_tol=0.0, abs_tol=1e-12):
            raise RuntimeError("MiniGrid DistShift reward semantics drifted")
        if terminated != (success or hazard):
            raise RuntimeError("MiniGrid DistShift termination semantics drifted")
        if truncated != (self._steps == self._max_steps):
            raise RuntimeError("MiniGrid DistShift horizon semantics drifted")
...
def _observation(value: object) -> tuple[dict[str, PolicyValue], _Facts]:
    ...
    try:
        ...
        raise RuntimeError("MiniGrid DistShift returned invalid direction") from error
...
def _observation_signature(
    ...
    if type(image) is not TensorValue or type(direction) is not int:
        raise RuntimeError("MiniGrid DistShift public observation is invalid")
