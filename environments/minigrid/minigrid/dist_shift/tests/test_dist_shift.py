...
class DistShiftTests(unittest.TestCase):
    ...
    def test_split_planning_and_scenario_rejection(self) -> None:
        ...
        with self.assertRaises(ValueError):
            benchmark.make_environment(EpisodeSpec(environment_seed=1, scenario={"strip2_row": 5}))
    ...
    def test_step_feedback_exposes_safety_and_exploration(self) -> None:
        ...
        try:
            ...
            self.assertEqual(step.metrics["remaining_steps"], 251)
            ...
            self.assertEqual(step.metrics["goal_found"], True)
            self.assertEqual(step.metrics["hazard_found"], True)
    ...
    def test_feedback_privacy(self) -> None:
        ...
        trace = DistShiftBenchmark().feedback((failed,)).artifacts[0].read_bytes()
    ...
    def test_real_hazard_and_time_limit_outcomes_are_distinct(self) -> None:
        ...
        hazard_record = EpisodeRecord(
            ...
            transitions=(
                Transition(action=2, step=first_step),
                Transition(action=2, step=hazard_step),
            ),
        )
        ...
        self.assertEqual(
            hazard_feedback.content["episodes_goal_found_but_not_reached"],
            1,
        )
    ...
    def test_baseline_solves_all_profiles_without_hazard(self) -> None:
        ...
        for profile in profiles:
            with self.subTest(profile=profile):
                benchmark = DistShiftBenchmark(DistShiftConfig(profile=profile))
                result = evaluate(
                    baseline_program(),
                    benchmark,
