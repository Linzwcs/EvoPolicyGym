...
class _EpisodeDiagnostics:
    ...
    target_first_in_front_step: int
    pickup_step: int
    ...
    front_candidate_before_action: str
    picked_up_label: str
    wrong_object_label: str
    task_stage: str
    outcome: str
...
class FetchBenchmark:
    ...
    def feedback(self, episodes: Sequence[EpisodeRecord]) -> Feedback:
        ...
        content: dict[str, PolicyValue] = {
            ...
            "mean_target_first_in_front_step": _mean_milestone(
                tuple(item.target_first_in_front_step for item in diagnostics)
            ),
            "mean_pickup_step": _mean_milestone(tuple(item.pickup_step for item in diagnostics)),
            ...
            "mean_pickup_attempt_count": _mean_or_none(
                tuple(float(item.pickup_attempt_count) for item in diagnostics)
            ),
            "mean_failed_pickup_count": _mean_or_none(
                tuple(float(item.failed_pickup_count) for item in diagnostics)
            ),
            "episodes_target_found_but_not_picked_up": sum(
                _reached_milestone(record, "target_found")
                and not _successful(record)
                and not _reached_milestone(record, "wrong_object")
                for record in records
            ),
            ...
            "failed_pickup_episodes": failed_pickups,
...
def _benchmark_spec(config: FetchConfig) -> BenchmarkSpec:
    return BenchmarkSpec(
        ...
        environment_parameters={
            ...
            "natural_termination": (
                "picking up any key or ball terminates; the mission target "
                "succeeds and a distractor returns zero; other actions do "
...
def _episode_outcome(record: EpisodeRecord) -> str:
    ...
    if type(reason) is not str:
        raise ValueError("MiniGrid Fetch terminal reason is invalid")
    return reason if reason != "none" else "incomplete"


def _episode_diagnostics(record: EpisodeRecord) -> _EpisodeDiagnostics:
    if not record.transitions:
        raise ValueError("MiniGrid Fetch diagnostics require a transition")
    ...
    return _EpisodeDiagnostics(
        ...
        target_first_in_front_step=_int_metric(
            final,
            "target_first_in_front_step",
        ),
        pickup_step=_int_metric(final, "pickup_step"),
        ...
        pickup_attempt_count=_int_metric(final, "pickup_attempt_count"),
        ...
        front_candidate_before_action=_string_metric(
            final,
            "front_candidate_before_action",
        ),
        picked_up_label=_string_metric(final, "picked_up_label"),
        wrong_object_label=_string_metric(final, "wrong_object_label"),
...
def _int_metric(metrics: dict[str, PolicyValue], name: str) -> int:
    ...
    if type(value) is not int:
        raise ValueError(f"MiniGrid Fetch metric {name} is invalid")
    return value
            ...
            _json_line(
                {
                    ...
                    "failed_pickup": _reached_milestone(
                        record,
                        "failed_pickup",
                    ),
                    ...
                    "target_first_in_front_step": (
                        diagnostics.target_first_in_front_step if diagnostics is not None else None
                    ),
                    "pickup_step": (diagnostics.pickup_step if diagnostics is not None else None),
                    "unique_candidate_count_found": (
                    ...
                    "front_candidate_before_action": (
                        diagnostics.front_candidate_before_action
                        if diagnostics is not None
                        else None
                    ),
                    "picked_up_label": (
                        diagnostics.picked_up_label if diagnostics is not None else None
                    ),
                    "wrong_object_label": (
                        diagnostics.wrong_object_label if diagnostics is not None else None
                    ),
                    "pickup_attempt_count": (
                        diagnostics.pickup_attempt_count if diagnostics is not None else None
                    ),
                    "failed_pickup_count": (
