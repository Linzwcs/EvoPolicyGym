...
_MISSIONS = tuple(f"go to the {color} door" for color in _COLOR_NAMES)
    ...
    return BenchmarkSpec(
        ...
        environment_parameters={
            ...
            "natural_termination": (
                ...
                "and succeeds only when adjacent to the mission door"
            ),
...
def _episode_outcome(record: EpisodeRecord) -> str:
    ...
    if type(reason) is not str:
        raise ValueError("MiniGrid GoToDoor terminal reason is invalid")
    return reason if reason != "none" else "incomplete"


def _episode_diagnostics(record: EpisodeRecord) -> _EpisodeDiagnostics:
    if not record.transitions:
        raise ValueError("MiniGrid GoToDoor diagnostics require a transition")
...
def _int_metric(metrics: dict[str, PolicyValue], name: str) -> int:
    ...
    if type(value) is not int:
        raise ValueError(f"MiniGrid GoToDoor metric {name} is invalid")
    return value
...
def _target(mission: str) -> tuple[str, str]:
    ...
    if len(parts) != 2 or parts[0] not in _COLOR_NAMES or parts[1] != "door":
        raise ValueError("MiniGrid GoToDoor trace mission is invalid")
