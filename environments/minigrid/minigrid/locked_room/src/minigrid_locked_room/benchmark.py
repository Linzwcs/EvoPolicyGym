...
_MISSIONS = tuple(
    f"get the {target} key from the {room} room, unlock the {target} door and go to the goal"
...
_METRICS = frozenset(
    {
        ...
        "matching_target_key_carried",
        "key_room_door_opened_this_step",
        "key_room_door_opened",
        "key_room_door_opened_step",
        "target_door_visible",
        "target_door_found",
        ...
        "visible_door_count",
        "unique_door_color_count_opened",
...
class _EpisodeDiagnostics:
    ...
    unique_door_color_count_opened: int
...
class LockedRoomBenchmark:
    ...
    def feedback(self, episodes: Sequence[EpisodeRecord]) -> Feedback:
        ...
        content: dict[str, PolicyValue] = {
            ...
            "mean_key_room_door_opened_step": _mean_milestone(
                tuple(item.key_room_door_opened_step for item in diagnostics)
            ),
...
def _spec() -> BenchmarkSpec:
    return BenchmarkSpec(
        ...
        environment_parameters={
            ...
            "natural_termination": (
                "moving forward onto the goal terminates with success; "
                "opening the target locked door is an intermediate milestone"
            ),
            "door_colors_unique": True,
            "time_limit": 190,
        },
...
def _episode_diagnostics(record: EpisodeRecord) -> _EpisodeDiagnostics:
    if not record.transitions:
        raise ValueError("MiniGrid LockedRoom diagnostics require a transition")
    final = _trace_metrics(record.transitions[-1].step.metrics)
    return _EpisodeDiagnostics(
        key_room_door_opened_step=_int_metric(
            final,
            "key_room_door_opened_step",
        ),
        ...
        unique_door_color_count_opened=_int_metric(
            final,
            "unique_door_color_count_opened",
        ),
        ...
        key_room_color=_string_metric(final, "key_room_color"),
...
def _int_metric(metrics: dict[str, PolicyValue], name: str) -> int:
    ...
    if type(value) is not int:
        raise ValueError(f"MiniGrid LockedRoom metric {name} is invalid")
    return value
    ...
    for episode_index, record in enumerate(records):
            ...
            _json(
                {
                    ...
                    "key_room_door_opened": _reached(
                        record,
                        "key_room_door_opened",
                    ),
                    "key_found": _reached(record, "key_found"),
                    "key_picked_up": _reached(record, "key_picked_up"),
                    "target_door_found": _reached(
                        record,
                        "target_door_found",
                    ),
                    "target_door_opened": _reached(
                        record,
                        "target_door_opened",
                    ),
                    ...
                    "key_room_color": (
                        diagnostics.key_room_color if diagnostics is not None else None
                    ),
                    "key_room_door_opened_step": (
                        diagnostics.key_room_door_opened_step if diagnostics is not None else None
                    ),
                    "key_picked_up_step": (
                    ...
                    "unique_door_color_count_opened": (
                        diagnostics.unique_door_color_count_opened
                        if diagnostics is not None
                        else None
                    ),
                    "door_open_event_count": (
                        diagnostics.door_open_event_count if diagnostics is not None else None
                    ),
                    "door_close_event_count": (
                        diagnostics.door_close_event_count if diagnostics is not None else None
                    ),
                    "front_object_before_action": (
        ...
        for index in indices:
            ...
            if type(item.action) is not int or not 0 <= item.action <= 6:
                raise ValueError("MiniGrid LockedRoom trace Action is invalid")
            lines.append(
                _json(
                    {
                        "type": "transition",
...
def _trace_metrics(metrics: PolicyValue) -> dict[str, PolicyValue]:
    if type(metrics) is not dict or set(metrics) != set(_METRICS):
        raise ValueError("MiniGrid LockedRoom trace metrics are invalid")
    boolean_fields = {
        "target_key_visible",
        "key_found",
        "key_picked_up_this_step",
