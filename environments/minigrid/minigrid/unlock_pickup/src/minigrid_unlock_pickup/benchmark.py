...
def _spec() -> BenchmarkSpec:
    return BenchmarkSpec(
            ...
            "fields": {
                "image": {
                    ...
                    "meaning": (
                        ...
                        "box is encoded at the agent position."
                    ),
                },
                "direction": {
                    "type": "integer",
        ...
        environment_parameters={
            ...
            "natural_termination": (
                "only picking up the mission box terminates; opening the "
                "door is an intermediate milestone, while toggling the "
                "mission box destroys it without terminating"
            ),
            "time_limit": 8 * 6**2,
        },
...
def _episode_diagnostics(record: EpisodeRecord) -> _EpisodeDiagnostics:
    if not record.transitions:
        raise ValueError("MiniGrid UnlockPickup diagnostics require a transition")
    final = _trace_metrics(record.transitions[-1].step.metrics)
...
def _boolean_metric(metrics: dict[str, PolicyValue], name: str) -> bool:
    ...
    if type(value) is not bool:
        raise ValueError(f"MiniGrid UnlockPickup metric {name} is invalid")
    return value
    ...
    for episode_index, record in enumerate(records):
        ...
        for index in indices:
            ...
            if type(item.action) is not int or not 0 <= item.action <= 6:
                raise ValueError("MiniGrid UnlockPickup trace Action is invalid")
            lines.append(
                _json(
                    {
                        "type": "transition",
...
def _trace_metrics(metrics: PolicyValue) -> dict[str, PolicyValue]:
    if type(metrics) is not dict or set(metrics) != set(_METRICS):
        raise ValueError("MiniGrid UnlockPickup trace metrics are invalid")
    boolean_fields = {
        "target_visible",
        "target_found",
        "target_in_front",
