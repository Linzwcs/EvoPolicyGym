...
def video_feedback(
    ...
    *,
    ...
    """Build replay GIFs and public manifests for a bounded Episode prefix."""
    ...
    for episode_index, record in enumerate(records):
        ...
        if not frames:
            manifests.append(
                {
                    "schema": "robosuite/rendered-frame-evidence/v1",
        ...
        name = f"episode-{episode_index:03d}/agentview.gif"
        artifacts.append(
            Artifact(
                name=name,
...
def _frame_evidence_artifact(
    ...
    *,
    ...
    if not frames:
        raise ValueError("robosuite frame evidence requires at least one frame")
    output = io.BytesIO()
    np.savez_compressed(
        output,
...
def video_capture_interval(max_episode_steps: int) -> int:
    """Choose one fixed stride that keeps initial/result evidence bounded."""
        ...
        if VIDEO_FRAME_METRIC in metrics:
            frames.append(
                _ReplayFrame(
                    ...
                    success=metrics.get("success_ever") is True,
                )
            )
    if len(frames) > VIDEO_MAX_FRAMES_PER_EPISODE:
        raise ValueError("robosuite video frame count exceeds its public bound")
...
def _frame(value: PolicyValue) -> NDArray[np.uint8]:
    if (
        ...
        raise ValueError("robosuite video metric contains an invalid RGB frame")
...
def _encode_bounded_gif(
    ...
    *,
    ...
    if not frames:
        raise ValueError("robosuite replay GIF requires at least one frame")
    ...
    while len(content) > _MAX_VIDEO_ARTIFACT_BYTES and len(selected) > 2:
        ...
        content = _gif_bytes(selected, profile=profile, scale=1)
    if len(content) > _MAX_VIDEO_ARTIFACT_BYTES:
        raise ValueError("robosuite replay GIF exceeds its public byte bound")
...
def _render_frame(
    ...
    *,
    ...
    camera = Image.fromarray(frame.frame, mode="RGB").resize(
        (width, height),
