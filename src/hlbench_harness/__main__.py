...
Two backends, selected via ``--backend {claude,codex}`` (default
``claude``):
...
Internally a run does:
  ...
  3. ``ClaudeAgent`` or ``CodexAgent`` with cwd = workspace.
...
def add_subparser_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        help="model id passed to the backend (default: 'sonnet' for "
             "claude, 'gpt-5-codex' for codex)",
    )
    p.add_argument(
        "--turn-timeout", type=int, default=None,
        help="seconds before a single agent turn times out (default: 600 "
             "for claude, 900 for codex — codex's first-call latency on "
             "macOS commonly busts 600)",
    )
    p.add_argument(
        "--session-id", default=None,
        help="reuse an explicit UUID for the agent session "
             "(default: auto-generated; codex backend mints a harness-side "
             "label here, the codex-internal id is scraped from turn 0)",
    )
    ...
    p.set_defaults(
        require_claude=True, require_codex=True,
        codex_bypass_approvals=True, func=run_with_args,
    )
...
def run_with_args(args: argparse.Namespace) -> int:
    ...
    if args.model is None:
        args.model = "sonnet" if backend == "claude" else "gpt-5-codex"
