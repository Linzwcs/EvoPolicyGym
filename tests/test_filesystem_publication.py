...
from evopolicygym.run import _publication as publication_module
...
class FilesystemSubmissionPublisherTests(unittest.TestCase):
    ...
    def test_failed_paired_eviction_restores_both_views(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            publisher.commit(old)
            workspace_old = feedback / "submissions" / old.submission_id
            replace_json = publication_module._replace_json
            ...
            with patch(
                "evopolicygym.run._publication._replace_json",
