...
class HarnessProtocolTest(unittest.TestCase):
    def test_epoch_zero_uses_submission_evaluation_layout(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((epoch_dir / "input" / "complexity.json").is_file())
            self.assertTrue((epoch_dir / "submission" / "complexity.json").is_file())
            ...
            self.assertIn("complexity", transition["input"])
            self.assertIn("complexity", transition["submission"])
            self.assertIn("complexity_delta", transition["comparison"])
            self.assertIn("sloc", transition["submission"]["complexity"])
            self.assertIn("branch_count", transition["submission"]["complexity"])
    ...
    def test_loop_writes_report_artifacts_outside_workspace(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("final_policy", metrics["complexity"])
            self.assertIn("growth_from_initial", metrics["complexity"])
            self.assertIn("policy_sloc", metrics["curves"])
            self.assertIn("policy_branch_count", metrics["curves"])
            self.assertIn("policy_sloc", curve[0])
